...
//	glGetShaderiv( shader, GL_COMPILE_STATUS, &ShaderDidCompile );
	glGetObjectParameterivARB( shader, GL_OBJECT_COMPILE_STATUS_ARB, &ShaderDidCompile );
...
//			glGetShaderInfoLog( shader, infoLen, NULL, infoLog->Data );
			glGetInfoLogARB( shader, infoLen, NULL, infoLog->Data );
			printf( "%s\n", infoLog->Data );
...
//		VertexProgram = CompileShader( ShaderSrc->Data, GL_VERTEX_SHADER );
		VertexProgram = CompileShader( ShaderSrc->Data, GL_VERTEX_SHADER_ARB );
...
//		FragmentProgram = CompileShader( ShaderSrc->Data, GL_FRAGMENT_SHADER );
		FragmentProgram = CompileShader( ShaderSrc->Data, GL_FRAGMENT_SHADER_ARB );
		delete_DataBlock( ShaderSrc );	
	}
...
//	glAttachShader( Program, FragmentProgram );
	...
	glAttachObjectARB( Program, VertexProgram );
	glAttachObjectARB( Program, FragmentProgram );

	// Variables //
	int Location = glGetUniformLocationARB( Program, "Tex0" ); 
...
//	glGetProgramiv( Program, GL_LINK_STATUS, &ProgramDidLink );
	glGetObjectParameterivARB( Program, GL_OBJECT_LINK_STATUS_ARB, &ProgramDidLink );
...
//		glGetProgramiv( Program, GL_INFO_LOG_LENGTH, &infoLen ); 
		glGetObjectParameterivARB( Program, GL_OBJECT_INFO_LOG_LENGTH_ARB, &infoLen ); 
		if ( infoLen > 1 ) {
			DataBlock* infoLog = new_DataBlock( infoLen );
			
//			glGetProgramInfoLog( Program, infoLen, NULL, infoLog->Data );
			glGetInfoLogARB( Program, infoLen, NULL, infoLog->Data );
			printf( "%s\n", infoLog->Data );
...
//	glDeleteProgram( Program );
	...
	glDeleteObjectARB( VertexProgram );
